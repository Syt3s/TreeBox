...
func Login(ctx context.Context, req LoginRequest, recaptcha recaptcha.RecaptchaV3) error {
	...
	if err != nil {
		logrus.WithContext(ctx.Request().Context()).WithError(err).Error("Failed to generate auth token")
...
func Register(ctx context.Context, req RegisterRequest, recaptcha recaptcha.RecaptchaV3) error {
	if conf.App.Production {
		resp, err := recaptcha.Verify(req.Recaptcha, ctx.Request().Request.RemoteAddr)
		if err != nil {
			logrus.WithContext(ctx.Request().Context()).WithError(err).Error("Failed to check recaptcha")
	...
	if err != nil {
		logrus.WithContext(ctx.Request().Context()).WithError(err).Error("Failed to get user after registration")
...
func AdminResetPassword(ctx context.Context, req AdminResetPasswordRequest) error {
	if conf.App.Production {
...
func UpdateProfile(ctx context.Context, req UpdateProfileRequest) error {
	...
	if strings.TrimSpace(req.NewPassword) != "" {
		...
		if err := db.Users.ChangePassword(ctx.Request().Context(), ctx.User.ID, req.OldPassword, req.NewPassword); err != nil {
			...
			logrus.WithContext(ctx.Request().Context()).WithError(err).Error("Failed to change password")
...
func UpdateHarassment(ctx context.Context, req UpdateHarassmentRequest) error {
	words := []string{}
...
func GetUserQuestions(ctx context.Context) error {
	...
	if err != nil {
		logrus.WithContext(ctx.Request().Context()).WithError(err).Error("Failed to get user questions")
...
func ExportData(ctx context.Context) error {
	...
	if err != nil {
		logrus.WithContext(ctx.Request().Context()).WithError(err).Error("Failed to export questions")
