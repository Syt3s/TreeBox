...
func authenticatedUser(c *gin.Context) *model.User {
	authz := c.Request.Header.Get("Authorization")
	if strings.HasPrefix(authz, "Bearer ") {
		token := strings.TrimSpace(strings.TrimPrefix(authz, "Bearer "))
		if token != "" {
			if claims, err := security.ParseToken(token); err == nil {
				if user, err := repository.Users.GetByID(c.Request.Context(), claims.UID); err == nil {
					return user
				}
			}
		}
	...
	return func(c *gin.Context) {
		...
		if options.UserSignInRequired && !ctx.IsLogged {
			if endpoint.IsAPI() {
				...
				ctx.Redirect(config.App.ExternalURL + "/login")
			}
			c.Abort()
			return
		}
